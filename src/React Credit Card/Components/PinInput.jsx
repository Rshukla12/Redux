...
const PinInput = ({ noOfBoxes = 4, length = 1, onChange, target }) => {
	...
	const handlePaste = (e) => {
		...
		val.forEach((char, index) => {
			values[Math.floor(index/length)] = char;
			ref.current[Math.floor(index/length)].value += char;
			if (Math.floor(index/length) < noOfBoxes - 1 && ref.current[Math.floor(index/length)].value===length) {
				ref.current[Math.floor(index/length) + 1].focus();
                setIsActive(Math.floor(index/length) + 1);
			}
			setValues([...values]);
			onChange(values.join(""));
		});
